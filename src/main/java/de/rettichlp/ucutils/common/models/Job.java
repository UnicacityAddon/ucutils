...
public enum Job {
    ...
    DEEP_SEA_FISHER("Hochseefischer", ofMinutes(20), compile("^\\[Fischer] Fahre nun zu den Fischschwärmen und wirf dein Fischenetz mit /catchfish aus\\.$")),
    WINEMAKER("Winzer", ofMinutes(20), compile("^\\[Winzer] Gehe nun zum Rebstock und sammel die Trauben\\.$")),
    TABLE_CLEANER("Tellerwäscher", ofMinutes(20), compile("^\\[TableCleaner] Du hast die Arbeit im Dönerladen als Putzkraft begonnen! Reinige \\d+ Tische\\.$")),
    GARDENER("Gärtner", ofMinutes(20), compile("^\\[Gärtner] Pflücke \\d+ Blumen an den markierten Stellen\\.$"));
